...
- Before constructing human mentions from identities, resolve the entire intended batch in one workspace with `user resolve`. Use returned mentions only when `safe_to_mention` is true; a nonzero, ambiguous, missing, or incomplete result is unsafe.
